...
class Config:
    ...
    def validate(self):
        ...
        if not self.openai_api_key and not self.google_gemini_api_key:
            raise ValueError("At least one of OPENAI_API_KEY or GOOGLE_GEMINI_API_KEY must be set")
        
        # Check Notion API key and database ID if using Notion storage
        if self.notion_api_key and not self.notion_database_id:
            raise ValueError("NOTION_DATABASE_ID must be set when NOTION_API_KEY is provided")