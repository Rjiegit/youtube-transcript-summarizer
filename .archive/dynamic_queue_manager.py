...
class DynamicQueueManager:
    ...
    def _process_test_task(task: Dict[str, Any]):
        ...
        try:
            ...
            # 測試模式下的檔案名處理
            timestamp = (
                now_iso().replace(":", "").replace("-", "").replace("T", "_")[:15]
            )
            ...
            file_result = FileManager.save_text(
                test_summary, output_file=final_filename
            )
            ...
            try:
                ...
                storage_result = storage.save(
                    file_title, test_summary, "gemini-2.0-flash", task["url"]
                )
    ...
    @staticmethod
    def add_url(
        raw_url: str, test_type: Optional[str] = None, sample_id: Optional[int] = None
    ) -> tuple[bool, str]:
        """新增任務到佇列。支援測試任務標記。"""
        ...
        if test_type:
            logger.info(
                f"[測試模式] 新增測試任務: {task_id} - {norm} (類型: {test_type})"
            )
