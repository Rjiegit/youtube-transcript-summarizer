...
class TestSummaryStorage(unittest.TestCase):
    ...
    def test_init_with_config(self, mock_client_class):
        ...
        self.assertEqual(
            summary_storage.notion_api_key, self.mock_config.notion_api_key
        )
        self.assertEqual(
            summary_storage.notion_database_id, self.mock_config.notion_database_id
        )
    ...
    def test_init_without_config(
        self, mock_client_class, mock_getenv, mock_load_dotenv
    ):
    ...
    def test_save_without_notion_client(self):
        ...
        summary_storage.save(
            self.test_title, self.test_text, self.test_model, self.test_url
        )
    ...
    def test_save_with_notion_client(self, mock_save_with_notion):
        ...
        mock_save_with_notion.assert_called_once_with(
            self.test_title, self.test_text, self.test_model, self.test_url
        )
    ...
    def test_save_with_notion(self, mock_split_text):
        ...
        with patch.object(
            ...
            self.assertEqual(
                call_args["parent"],
                {"database_id": self.mock_config.notion_database_id},
            )

            # Check properties
            properties = call_args["properties"]
            self.assertEqual(
                properties["Title"]["title"][0]["text"]["content"], self.test_title
            )
            self.assertEqual(properties["URL"]["url"], self.test_url)
            self.assertEqual(
                properties["Model"]["rich_text"][0]["text"]["content"], self.test_model
            )
            ...
            for i, chunk in enumerate(text_chunks):
                self.assertEqual(
                    children[i]["paragraph"]["rich_text"][0]["text"]["content"], chunk
                )
