...
sources:
  ...
  - id: openwiki-source-349a53f3765c7ed13452a632
    resource: repo://frontend/nuxt-showcase/AGENTS.md
...
- `frontend/nuxt-showcase/` 是獨立的 Nuxt 3 展示站，使用自己的 TypeScript、Vue 與 Vitest 規則。
...
Nuxt 使用 Vitest、Vue Test Utils 與 jsdom；測試放在 `frontend/nuxt-showcase/tests/`，命名為 `*.test.ts`。優先測試資料轉換、日期格式化、API handler、SWR cache 與 read state。
