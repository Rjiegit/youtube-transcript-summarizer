...
describe("showcase env resolution", () => {
  ...
  it("resolves from direct NOTION_* process env", () => {
    ...
    process.env.NOTION_URL = "https://www.notion.so/workspace";
    ...
    expect(resolved.notionUrl).toBe("https://www.notion.so/workspace");
  ...
  it("falls back to NUXT_* env names", () => {
    ...
    expect(resolved.notionUrl).toBe("https://www.notion.so/nuxt-workspace");
