...
class Main:
    ...
    def run(self):
        ...
        FileManager.save_text(transcription_text, output_file=f"data/{file_title}.txt")
        
        summarized_text = summarizer.summarize(transcription_text)
        FileManager.save_text(summarized_text, output_file=f"data/summarized_{file_title}_summary.md")
