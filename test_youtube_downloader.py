...
class TestYouTubeDownloader(unittest.TestCase):
    def setUp(self):
        ...
        self.test_video_path = os.path.join(self.test_output_path, "videos", "test_video.mp4")
    ...
    def test_download_with_yt_dlp(self, mock_listdir, mock_getctime, mock_run):
        ...
        mock_run.assert_called_once_with([
            "yt-dlp",
            "-S", "res:360",
            "-o", os.path.join(self.test_output_path, "videos", '%(title)s.%(ext)s'),
            self.test_url
        ], check=True)
    ...
    def test_download_with_yt_dlp_no_files(self, mock_listdir):
        ...
        self.assertEqual(str(context.exception), "yt-dlp did not download any video file.")
    ...
    def test_download_with_pytube(self, mock_youtube_class):
        ...
        expected_result = {
            ...
            "title": self.test_video_title
        }
        self.assertEqual(result, expected_result)
        
        # Verify that YouTube was called with the correct parameters
        mock_youtube_class.assert_called_once_with(self.test_url, on_progress_callback=unittest.mock.ANY)
