...
class TestSummarizer(unittest.TestCase):
    ...
    def test_get_prompt(self, mock_prompt):
        ...
        mock_prompt.format.assert_called_once_with(title=self.test_title, text=self.test_text)
    ...
    def test_summarize_with_google_gemini(self, mock_get_prompt, mock_genai):
        ...
        # Call the method under test
        result = self.summarizer.summarize_with_google_gemini(self.test_title, self.test_text)
        
        # Verify the result
        self.assertEqual(result, self.test_summary)
        
        # Verify that the API key was configured
        mock_genai.configure.assert_called_once_with(api_key=self.mock_config.google_gemini_api_key)
    ...
    def test_summarize_with_ollama_error(self, mock_get_prompt, mock_post):
        ...
        self.assertIn("Error from Ollama API: 500 Internal Server Error", str(context.exception))
    ...
    def test_summarize(self, mock_summarize_with_google_gemini):
        ...
        mock_summarize_with_google_gemini.assert_called_once_with(self.test_title, self.test_text)
    ...
    def test_init_with_config(self):
        ...
        self.assertEqual(summarizer.google_gemini_api_key, self.mock_config.google_gemini_api_key)
